                            ...
                            <td>
                                <div class="d-flex flex-row">
                                    ...
                                    @for ($i = 0; $i < $days; $i++)
                                        ...
                                        $curr=App\Models\ExtraWork::where( 'employees_detail_id' ,
                                        $employee->id,
                                        )
                                        ->where('date', $date)
                                        ->first();
                                        @endphp
                                        <div
                                            class="p-2   {{ in_array($date, $employee->extra_work_dates) ? 'bg-success' : (in_array($date, $employee->leave_dates) ? 'bg-dark text-white' : (now()->isAfter($currentYear . '-' . $currentMonth . '-' . sprintf('%02d', $i + 1)) ? 'bg-danger' : 'bg-secondary')) }}">
                                            {{ sprintf('%02d', $i + 1) }}
                                            @php
